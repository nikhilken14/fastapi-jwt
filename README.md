...
1. **All 8 tests failed with `AttributeError: 'async_generator' object has
   no attribute 'post'/'get'`.** The `client` fixture in
   `app/tests/test_auth.py` is an `async def` generator fixture, but the
   project had no pytest-asyncio configuration and the fixture was declared
   with plain `@pytest.fixture`. Without `asyncio_mode = auto` (or an
   equivalent `@pytest_asyncio.fixture` decorator), pytest-asyncio never
   awaits the fixture, so `client` in each test was the raw async generator
   object instead of the yielded `AsyncClient`. **Fix:** added `pytest.ini`
   with `asyncio_mode = auto` and `asyncio_default_fixture_loop_scope =
   function`. All 8 tests now pass.
2. **`auth.db` (a real SQLite file with data) was committed to the repo**,
   and nothing ignored it or other generated artifacts (`__pycache__/`,
   `.pytest_cache/`, `.env`). **Fix:** removed `auth.db` from the project and
   added a `.gitignore`.
3. **Missing `.env.example`.** `app/core/config.py`'s docstring refers
   operators to `.env.example` "for the full list [they] can override," but
   the file didn't exist. **Fix:** added `.env.example` matching every field
   in `Settings`.

### Things that are correct but worth knowing about

- `SECRET_KEY` ships with an insecure development default
  (`insecure-dev-secret-change-me`). This is intentional for zero-config
  local runs, but **must** be overridden via a real environment variable
  (or `.env`, which is gitignored) in any shared/staging/production
  deployment — `docker-compose.yml` already supports this via
  `${SECRET_KEY:-...}`.
- Schema is created with `Base.metadata.create_all` at startup rather than
  Alembic migrations — fine for this demo's scope, called out in the code
  comments as a deliberate simplification.
- `password_hash` is deliberately excluded from `UserOut`, so it can never
  leak through an API response even if a handler is careless.